...
The tight clustering of the top seven losses (less than 0.5 percentage point spread) reflects
...
At $\eta{=}0.4$, SDIV degrades only 1.5 percentage points (from 98.12\% to
96.62\%), while CCE degrades 3.6 percentage points (from 98.25\% to 94.68\%).
...
losses is:
SDIV $-0.62$ percentage points, CCE $+0.39$ percentage points,
SCE $+0.21$ percentage points, FCL $-0.62$ percentage points, GCE $-0.61$ percentage points.
...
Table~\ref{tab:fgsm} and Figures~\ref{fig:fgsm_pathmnist} and~\ref{fig:fgsm_dermamnist}
...
CCE drops from 83.31\% to 17.60\% (a fall of 65.7 percentage points),
SDIV from 80.93\% to 15.89\% (a fall of 65.0 percentage points), and
FCL from 76.88\% to 7.12\% (a fall of 69.8 percentage points).
...
training~\citep{madry2018pgd}, do not confer FGSM robustness on this dataset.

\paragraph{DermaMNIST.}
A striking pattern emerges on DermaMNIST.
SDIV, MAE, and GCE maintain constant accuracy across all $\varepsilon$ values
(zero accuracy change), while CCE collapses from 72.27\% to 22.69\%
(a fall of 49.6 percentage points).
...
67.18\% to 46.63\% at $\varepsilon{=}8/255$ (a fall of 20.6 percentage points) and
SCE drops from 71.22\% to 54.11\% (a fall of 17.1 percentage points),
both substantially better than CCE which falls by 49.6 percentage points.
...
Figure~\ref{fig:sdiv_grid} (heatmaps for PathMNIST and DermaMNIST).
...
The \textbf{Emotion} dataset~\citep{saravia2018} (\texttt{dair-ai/emotion} on
Hugging Face; 6 emotion classes: sadness, joy, love, anger, fear, surprise)
provides 16{,}000 training samples, of which 1{,}500 are used here.
The \textbf{PubMedQA} dataset~\citep{jin2019pubmedqa} (\texttt{qiaojin/PubMedQA},
config \texttt{pqa\_labeled}; 3 classes: yes/no/maybe) provides 1{,}000 labelled
training questions; 1{,}500 samples are used (with repetition).
...
The absolute differences across losses are small (at most 1.0 percentage point on Emotion,
at most 2.7 percentage points on PubMedQA).
...
\begin{enumerate}[leftmargin=*,itemsep=4pt]
  ...
  \item \textbf{FGSM only; no PGD, CW, or DeepFool.}
        The proposal specifies evaluation under Projected Gradient
        Descent (PGD)~\citep{madry2018pgd}, Carlini-Wagner
        (CW)~\citep{carlini2017cw}, and DeepFool~\citep{moosavi2016deepfool}
...
Table~\ref{tab:summary} and Figure~\ref{fig:master_summary} collect key accuracy figures across datasets and
