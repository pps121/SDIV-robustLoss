...
CFG = dict(
    ...
    # ── Vision ViT (Part A) ───────────────────────────────────────────────────
    VIT_DATASETS    = ['mnist', 'pathmnist', 'dermamnist'],  # also 'fashion_mnist', 'cifar10'
    VIT_EPOCHS      = int(os.environ.get('ROBUST_NN_VIT_EPOCHS',   '30')),   # paper: 250
    VIT_BATCH       = int(os.environ.get('ROBUST_NN_VIT_BATCH',   '0')),    # 0 = auto-scale by VRAM
    VIT_SEEDS       = [42],
    VIT_PATCH       = 8,
    VIT_D_MODEL     = 64,
    VIT_HEADS       = 4,
...
# ── GPU performance: maximise utilisation on A40 / A100 / H100 ────────────────
if DEVICE.type == 'cuda':
    torch.backends.cudnn.benchmark        = True   # auto-tune fastest conv per input size
    torch.backends.cudnn.deterministic    = False  # allow non-deterministic for speed
    torch.backends.cuda.matmul.allow_tf32 = True   # TF32 matmul (Ampere+: ~2× faster)
    torch.backends.cudnn.allow_tf32       = True   # TF32 convolutions
    torch.set_float32_matmul_precision('high')      # same effect globally

# ── DataLoader worker count (env-var override; cap at 4 for RunPod safety) ────
_N_WORKERS = int(os.environ.get(
    'ROBUST_NN_NUM_WORKERS',
    str(min(4, max(0, (os.cpu_count() or 2) - 1))) if DEVICE.type == 'cuda' else '0',
))
_PREFETCH = 2 if _N_WORKERS > 0 else None


def _auto_batch_vit() -> int:
    """Return optimal ViT batch size from GPU VRAM.  Larger batch = higher utilisation."""
    if CFG['VIT_BATCH'] > 0:          # explicit env-var override keeps its value
        return CFG['VIT_BATCH']
    if not torch.cuda.is_available():
        return 64
    vram = torch.cuda.get_device_properties(0).total_memory / 1024 ** 3
    if   vram >= 40:  return 2048   # A100-80GB, A6000 48GB, L40S 48GB
    elif vram >= 24:  return 1024   # A100-40GB, A40 48GB, L40 48GB
    elif vram >= 16:  return 512    # RTX 4090 24GB, A10G 24GB, L4 24GB
    elif vram >= 10:  return 256    # T4 16GB, RTX 3080/3090
    else:             return 128    # 8 GB cards


CFG['VIT_BATCH'] = _auto_batch_vit()
_gpu_info = (f"  TF32=ON  cudnn.benchmark=ON  workers={_N_WORKERS}"
             if DEVICE.type == 'cuda' else f"  workers={_N_WORKERS}")
print(f"[GPU-Opt] ViT batch={CFG['VIT_BATCH']}{_gpu_info}")


def _maybe_compile(model: nn.Module) -> nn.Module:
    """Wrap with torch.compile for ~20-40 % speedup on Ampere/Hopper (PyTorch >= 2.0).
    Falls back silently on unsupported environments."""
    tv = tuple(int(x) for x in torch.__version__.split('+')[0].split('.')[:2])
    if tv >= (2, 0) and DEVICE.type == 'cuda':
        try:
            return torch.compile(model, mode='default', fullgraph=False)
        except Exception:
            pass  # compile unsupported in this env (e.g. Windows, older CUDA)
    return model
...
def build_vit(img_size: int, in_ch: int, num_classes: int) -> VisionTransformer:
    ...
    return _maybe_compile(model)

# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 5 ── DATASET LOADING (Vision)
# ═══════════════════════════════════════════════════════════════════════════════

def _load_medmnist_dataset(name: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Load PathMNIST / DermaMNIST via the medmnist library (already required for Part C).
    Reads the internal .imgs / .labels numpy arrays directly — no slow per-image iteration.
    Returns (X_train, y_train, X_test, y_test) with X shape (N, C, 32, 32) float32 in [0,1].
    """
    import medmnist
    from medmnist.info import INFO as MEDMNIST_INFO

    key = name.lower()   # 'pathmnist' or 'dermamnist'
    info = MEDMNIST_INFO[key]
    DataClass = getattr(medmnist, info['python_class'])
    print(f"  Loading {name.upper()} via medmnist (download if needed) ...")

    def _split_to_arrays(split: str):
        ds = DataClass(split=split, download=True, size=28)
        X = ds.imgs.astype('float32') / 255.0        # (N, 28, 28) or (N, 28, 28, 3)
        y = np.squeeze(ds.labels).astype(np.int64)   # (N,) — medmnist labels are (N,1)
        if X.ndim == 3:                               # grayscale → add channel dim
            X = X[:, np.newaxis, :, :]
        else:
            X = X.transpose(0, 3, 1, 2)              # (N,H,W,C) → (N,C,H,W)
        X = np.pad(X, ((0, 0), (0, 0), (2, 2), (2, 2)), mode='constant')  # 28→32
        return X, y

    X_tr, y_tr = _split_to_arrays('train')
    X_te, y_te = _split_to_arrays('test')
...
def load_vision_dataset(name: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Load vision dataset, return (X_train, y_train, X_test, y_test) as numpy arrays.
    X shape: (N, C, H, W) float32 in [0,1].  y shape: (N,) int64.
    Supports: mnist, fashion_mnist, cifar10 (HuggingFace) and
              pathmnist, dermamnist (medmnist — faster, offline after first download)."""
    if name in ('pathmnist', 'dermamnist'):
        return _load_medmnist_dataset(name)
...
def train_one(model: nn.Module,
    ...
    _dl_kw = dict(num_workers=_N_WORKERS, pin_memory=_pin,
                  persistent_workers=(_N_WORKERS > 0),
                  prefetch_factor=(_PREFETCH if _N_WORKERS > 0 else None))
    tr_loader = DataLoader(tr_ds, batch_size=batch_size, shuffle=True,  **_dl_kw)
    te_loader = DataLoader(te_ds, batch_size=batch_size, shuffle=False, **_dl_kw)
...
def run_vision_battery(dataset_name: str) -> None:
    ...
    # Descriptive class names for medical datasets; numeric fallback for others
    _med_keys = {'pathmnist': 'PathMNIST', 'dermamnist': 'DermaMNIST'}
    _med_cls = MEDMNIST_PROMPTS.get(_med_keys.get(dataset_name, ''), None)
    if _med_cls is not None:
        class_names = [_med_cls[i] for i in range(num_classes)]
    else:
...
def run_curriculum_annealing(dataset_name: str,
    ...
    tr_loader = DataLoader(NumpyImageDataset(X_tr, y_noisy),
                           CFG['VIT_BATCH'], shuffle=True,  **_dl_kw)
    te_loader = DataLoader(NumpyImageDataset(X_te, y_te),
                           CFG['VIT_BATCH'], shuffle=False, **_dl_kw)
