...
MARKERS = {
    "CCE":"o","MAE":"^","GCE":"s","GCE(q=0.7)":"s","TruncGCE":"D",
    "SCE":"v","TPDD-CCE":"P","TDPDSCCE":"P","TSCCE":"X","FCL":"*",
    "RKLD":"h","SDIV":"o","ForwardT":"d",
}
LSTYLES = {
    "CCE":"-","MAE":"--","GCE":":","GCE(q=0.7)":":","TruncGCE":"-.","SCE":"--",
    "TPDD-CCE":"-.","TDPDSCCE":"-.","TSCCE":":","FCL":"-","RKLD":":","SDIV":"-",
    "ForwardT":"--",
}
DISPLAY = {"GCE(q=0.7)":"GCE (q=0.7)","TDPDSCCE":"TPDD-CCE","ForwardT":"ForwardT\u2020"}
def dn(l): return DISPLAY.get(l, l)

def lkw(loss, lw=1.5, ms=5.5, alpha=1.0):
    c = COLORS.get(loss, "#444")
    bold = (loss == "SDIV")
    return dict(
        color=c, marker=MARKERS.get(loss, "o"),
...
MNIST_PAPER = {
    "CCE":0.9822,"SDIV":0.9801,"TDPDSCCE":0.9844,"TSCCE":0.9175,
    "GCE":0.9799,"SCE":0.9830,"FCL":0.9846,"RKLD":0.9798,
}
DERM_MAJORITY = 0.6688279301745635
NOISE_VALS    = [0, 10, 20, 30, 40]

# ── Load all CSVs ─────────────────────────────────────────────────────────────
def _csv(name): return pd.read_csv(CSVDIR / name)
...
def _clean(df):
    return {r["loss"]: r["accuracy"]
            for _, r in df[df.noise_rate == 0].iterrows()}
...
def plot_f01():
    ...
    for ax, (title, data, order, note) in zip(axes, datasets):
        ...
        bars = ax.bar(range(len(avail)), vals, color=cols,
                      width=0.68, alpha=0.86, edgecolor="white", linewidth=0.5)

        cce_val = _get(data, "CCE")
        if cce_val:
            ax.axhline(cce_val * 100, color="#000", linewidth=1.1,
                       linestyle="--", alpha=0.35, zorder=1)

        for i, (l, bar) in enumerate(zip(avail, bars)):
            if l == "SDIV":
                bar.set_edgecolor("#a00000"); bar.set_linewidth(2.0)
                is_derm = "DermaMNIST" in title
                if is_derm and vals[i] < 69:
                    ax.text(i, vals[i] + 0.3, f"{vals[i]:.1f}%\ncollapse",
                            ha="center", fontsize=7.5, color="#a00000",
                            fontweight="bold", va="bottom")
                else:
                    ax.text(i, vals[i] + 0.2, f"{vals[i]:.1f}%",
                            ha="center", fontsize=7.5, color="#a00000",
                            fontweight="bold", va="bottom")

        vv = [v for v, l in zip(vals, avail)
              if not ("DermaMNIST" in title and l == "SDIV" and v < 69)]
        if vv:
            best_v = max(vv)
            best_i = vals.index(best_v)
            ax.annotate("best", xy=(best_i, best_v),
                        xytext=(best_i, best_v + (max(vals) - min(vals)) * 0.18 + 0.3),
                        ha="center", fontsize=7.5, color=cols[best_i],
                        fontweight="bold",
                        arrowprops=dict(arrowstyle="->", color=cols[best_i], lw=0.7))

        if "DermaMNIST" in title:
            ax.axhline(73.32, color="#D62728", linewidth=1.0, linestyle=":",
                       alpha=0.7)
            ax.text(len(avail) - 0.5, 73.6,
                    "SDIV(opt)\nbeta=0.1,lam=-0.4\n=73.3%",
        ...
        ax.set_xticklabels([dn(l) for l in avail], rotation=45, ha="right",
                           fontsize=8.5)
        ...
        ax.text(0.01, 0.01, note, transform=ax.transAxes,
                fontsize=7.5, color="#555", va="bottom", style="italic")
...
# ============================================================================
def plot_f02():
    order = ["CCE","GCE(q=0.7)","TruncGCE","SCE","TPDD-CCE","TSCCE","FCL",
             "ForwardT","SDIV"]
    ...
    ax_main.legend(loc="upper right", ncol=1, fontsize=8.5)
    ...
    ax_mae.set_title("MAE: Gradient Instability", fontsize=11, color="#E69F00",
                     fontweight="bold")
    ax_mae.axhline(1/9 * 100, color="#aaa", linewidth=0.8, linestyle=":",
                   label="Random baseline (11.1%)")
    ax_mae.legend(fontsize=8.5)
    ax_mae.text(0.5, 0.52,
                "Collapses eta=10-30%\nthen recovers at 40%.\nGradient instability,\nnot noise tolerance.",
                transform=ax_mae.transAxes, fontsize=8.5, ha="center",
                color="#E69F00",
...
def plot_f03():
    ...
    for loss in order_degen:
        ...
        if sub.empty:
            continue
        ax.plot(sub.noise_rate.values * 100, sub.accuracy.values * 100,
                **lkw(loss, lw=1.5))
    ax.axhline(DERM_MAJORITY * 100, color="#999", linewidth=1.4, linestyle=":",
    ...
    for loss in order_healthy:
        ...
        if sub.empty:
            continue
        ax2.plot(sub.noise_rate.values * 100, sub.accuracy.values * 100,
                 **lkw(loss, lw=1.5))
    ax2.scatter([0], [73.32], marker="*", s=260, color="#D62728", zorder=9,
                label="SDIV(opt) beta=0.1, lam=-0.4 (eta=0 only)")
    ax2.axhline(DERM_MAJORITY * 100, color="#999", linewidth=1.0, linestyle=":",
                label=f"Majority baseline ({DERM_MAJORITY*100:.1f}%)")
    ...
    ax2.set_ylim(64, 78)
    ax2.legend(loc="lower left", fontsize=8.5, ncol=1)
    ax2.set_title("(b) Non-Degenerate Losses + Optimised SDIV\n(FCL and TSCCE most noise-robust)",
                  fontsize=10.5)
    ax2.text(0.99, 0.02, "\u2020 ForwardT requires oracle noise matrix",
             transform=ax2.transAxes, fontsize=7.5, ha="right",
             color=COLORS["ForwardT"], style="italic")
...
def plot_f04():
    ...
    for loss in order:
        ...
        ax.annotate(f"{ys[-1]:.2f}%",
                    xy=(40, ys[-1]),
                    xytext=(41.5, ys[-1]),
                    fontsize=8.5, color=COLORS.get(loss, "#333"),
                    va="center",
    ...
    for loss, col in [("CCE", "#000"), ("SDIV", "#D62728")]:
        ...
        drop = y0 - y4
        ax.annotate(f"Delta={drop:.1f}pp",
                    xy=(20, (y0 + y4) / 2),
...
def plot_f05():
    ...
    for loss in order:
        ...
        ax.annotate(f"{acc_arr[-1]*100:.0f}%",
                    xy=(8, acc_arr[-1] * 100),
    ...
    ax.axhline(1/9 * 100, color="#bbb", linewidth=0.8, linestyle=":",
               label=f"Random (9-class, {1/9*100:.0f}%)")
    ax.text(4.5, 25,
...
def plot_f06():
    ...
    for loss in degen:
        ...
        if sub.empty:
            continue
        ax.plot(sub.epsilon.values * 255, sub.accuracy.values * 100,
                **lkw(loss, lw=1.8))
    ax.axhline(DERM_MAJORITY * 100, color="#999", linewidth=1.0, linestyle=":",
               label=f"Majority baseline ({DERM_MAJORITY*100:.1f}%)")
    ax.set_xticks([0, 1, 2, 4, 8])
    ax.set_xticklabels(["0", "1/255", "2/255", "4/255", "8/255"])
    ...
    ax.set_ylim(63, 73)
    ax.legend(loc="upper right", fontsize=9)
    ax.set_title("(a) Degenerate Losses: Flat Curves = Collapse Artefact\n"
    ...
    for loss in healthy:
        ...
        ax2.annotate(f"-{drop:.0f}pp",
                     xy=(8, acc_arr[-1] * 100),
                     xytext=(8.3, acc_arr[-1] * 100),
                     fontsize=7.5, va="center", color=COLORS.get(loss, "#444"))
    ax2.axhline(DERM_MAJORITY * 100, color="#999", linewidth=1.0, linestyle=":",
                label=f"Majority baseline ({DERM_MAJORITY*100:.1f}%)")
    ...
    ax2.set_title("(b) Non-Degenerate Losses: Genuine but Limited Resilience\n"
                  "(annotated: accuracy drop eps=0 to eps=8/255)", fontsize=10.5)
...
def plot_f07():
    def _pivot(df):
        return (df.pivot_table(index="beta", columns="lam",
    ...
    im = ax.imshow(piv_path.values, cmap=cmap_p, aspect="auto",
                   vmin=77, vmax=85)
    for i, b in enumerate(betas_p):
        for j, l in enumerate(lams_p):
            v = piv_path.loc[b, l]
            star = "* " if v == piv_path.values.max() else ""
            ax.text(j, i, f"{star}{v:.1f}", ha="center", va="center",
                    fontsize=9.5,
    ...
    im2 = ax2.imshow(piv_derm.values, cmap=cmap_d, aspect="auto",
                     vmin=66, vmax=74)
    for i, b in enumerate(betas_d):
        for j, l in enumerate(lams_d):
            ...
            ax2.text(j, i, f"{mark} {v:.1f}", ha="center", va="center",
                     fontsize=9,
...
def plot_f08():
    ...
    def _bar(ax, adict, title, majority_aunrc):
        ...
        bars = ax.barh(range(len(ls)), vs, color=cols,
                       height=0.68, alpha=0.85, edgecolor="white", linewidth=0.5)
        ax.axvline(majority_aunrc / 0.4 * 100, color="#999", linewidth=1.2,
                   linestyle="--", alpha=0.7, label="Majority baseline")

        for i, (l, v, b) in enumerate(zip(ls, vs, bars)):
            if l == "SDIV":
                b.set_edgecolor("#a00000"); b.set_linewidth(2.0)
            rank = i + 1
            ax.text(v + 0.1, i,
                    f"#{rank}  {v:.2f}%",
            ...
            if v < majority_aunrc / 0.4 * 100 + 1.0:
                ax.text(ax.get_xlim()[0] + 0.5, i,
                        "collapse", va="center", fontsize=7.5,
                        color="#aa0000", fontstyle="italic")
    ...
    _bar(ax1, apath, "PathMNIST AUNRC Ranking",
         majority_aunrc=1/9 * 0.4)
    _bar(ax2, aderm, "DermaMNIST AUNRC Ranking",
         majority_aunrc=DERM_MAJORITY * 0.4)
...
def plot_f09():
    ...
    def _nlp_bar(ax, df, title, note):
        ...
        bars = ax.bar(range(len(names)), vals, color=cols,
                      width=0.68, alpha=0.85, edgecolor="white", linewidth=0.5)
        if cce_ref is not None:
            ax.axhline(cce_ref * 100, color="#000", linewidth=1.0,
                       linestyle="--", alpha=0.35, label=f"CCE={cce_ref*100:.1f}%")

        for i, (n, v, b) in enumerate(zip(names, vals, bars)):
            if n == "SDIV":
                b.set_edgecolor("#a00000"); b.set_linewidth(2.0)
            ax.text(i, v + 0.05, f"{v:.1f}%", ha="center", va="bottom",
                    fontsize=8.5, color=COLORS.get(n, "#333"),
                    fontweight="bold" if n == "SDIV" else "normal")

        ax.set_xticks(range(len(names)))
        ax.set_xticklabels([dn(n) for n in names], rotation=40, ha="right",
                           fontsize=9)
...
def plot_f10():
    ...
    for loss in ["CCE", "GCE(q=0.7)", "SCE", "FCL", "SDIV"]:
        ...
        if sub.empty:
            continue
        ax_a.plot(sub.noise_rate.values * 100, sub.accuracy.values * 100,
                  **lkw(loss, lw=1.5, ms=5))
    ...
    ax_a.set_title("(a) PathMNIST Noise\nSDIV competitive — all losses >80% at eta=40%",
                   fontsize=10)
    ...
    for loss in ["CCE", "FCL", "TSCCE", "SCE", "SDIV"]:
        ...
        if sub.empty:
            continue
        ax_b.plot(sub.noise_rate.values * 100, sub.accuracy.values * 100,
                  **lkw(loss, lw=1.5, ms=5))
    ax_b.axhline(DERM_MAJORITY * 100, color="#999", linewidth=1.0, linestyle=":",
                 label=f"Majority ({DERM_MAJORITY*100:.1f}%)")
    ax_b.scatter([0], [73.32], marker="*", s=220, color="#D62728", zorder=10,
                 label="SDIV(opt) lam=-0.4")
    ...
    bars  = ax_d.barh(range(len(l_s)), v_s,
                      color=[COLORS.get(l, "#888") for l in l_s],
                      height=0.65, alpha=0.85)
    for i, (l, v, b) in enumerate(zip(l_s, v_s, bars)):
        if l == "SDIV":
            b.set_edgecolor("#a00000"); b.set_linewidth(1.8)
        ax_d.text(v + 0.1, i, f"#{i+1} {v:.1f}%", va="center", fontsize=8,
                  color=COLORS.get(l, "#333"),
                  fontweight="bold" if l == "SDIV" else "normal")
